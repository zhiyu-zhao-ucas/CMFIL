...
class RPS(gym.Env):
    def __init__(self, horizon):
        ...
        self.state = np.random.choice([0, 1, 2], p=np.ones(self.observation_space.n)/self.observation_space.n)
        self.count = 0
        self.horizon = horizon
        A = np.matrix([[4, 1, -4], [7, -6, -2], [1, 1, 1]])
        b = np.matrix([[0], [0], [1]])
        self.dist = torch.from_numpy(np.array(A.I @ b).T.squeeze())
        # [0.3377, 0.2597, 0.4026]
    ...
    def expert_data(self, obs):
        exp_signal = torch.tensor(0., dtype=torch.float)
        # if self.count == 0:
        ...
        exp_action = Categorical(probs=self.dist).sample().unsqueeze(-1)
        expert = torch.zeros(6)
        expert[:3] = torch.from_numpy(obs)
